...
export interface IHttp extends IMethods {
  ...
  proxy(url: string, data: object, method: String, urlParema?: boolean, headers?: Object): any;
}
...
class Http extends Methods implements IHttp {
  ...
  proxy(url: string, data: object, method: String, urlParema: boolean = false, headers: Object): any {
    //表示该接口正在调用
    let httpOpts: any = {
      url: super.getUrl(url),
      method: method,
...
axios.interceptors.request.use(
  function(config: any) {
    ...
    if (local && !config.noToken && !isToken) {
    ...
    if (error.response) {
      switch (error.response.status) {
        case 401:
          ...
          locStorage.removeItem('at_token');
          //app
          dsBridge.call(
            'jstowebview',
            {
